...
import { Flame, Zap, Filter, Share2, ChevronDown, X, Leaf, MapPin, ShoppingBag } from 'lucide-react';
...
function App() {
  ...
  const [selectedClassId, setSelectedClassId] = useState(() => {
    ...
    return classesData.some(c => c.id === classParam) ? classParam! : classesData[0].id;
  });
  ...
  const [selectedTab, setSelectedTab] = useState<TabType>(() => {
    ...
    return (tabParam && ['armor', 'spells'].includes(tabParam)) ? tabParam : 'armor';
  });
  
  const [selectedActId, setSelectedActId] = useState(() => {
    const params = new URLSearchParams(window.location.search);
    const actParam = params.get('act');
    return selectedClass.acts.some(a => a.id === actParam) ? actParam! : selectedClass.acts[0].id;
  });
  
  const selectedAct = selectedClass.acts.find(a => a.id === selectedActId) || selectedClass.acts[0];
  ...
  const [selectedLocation, setSelectedLocation] = useState<LocationType>(() => {
    ...
    return (locationParam && ['all', 'vendor', 'quest', 'loot', 'craft', 'boss'].includes(locationParam)) 
      ? locationParam 
      : 'all';
  });
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (slotDrawerRef.current && !slotDrawerRef.current.contains(event.target as Node)) {
        setShowSlotDrawer(false);
      }
      if (locationDrawerRef.current && !locationDrawerRef.current.contains(event.target as Node)) {
  ...
  }, [selectedClassId, selectedTab, selectedActId, selectedSlot, selectedLocation]);
  ...
  const handleClassChange = (classId: string) => {
    ...
    const newClass = classesData.find(c => c.id === classId) || classesData[0];
  ...
  const locationOptions: { value: LocationType; label: string; icon: JSX.Element }[] = [
    { value: 'all', label: 'All Locations', icon: <MapPin size={16} className="text-purple-400" /> },
    { value: 'vendor', label: 'Vendors', icon: <ShoppingBag size={16} className="text-purple-400" /> },
    { value: 'quest', label: 'Quest Rewards', icon: <Zap size={16} className="text-purple-400" /> },
    { value: 'loot', label: 'Loot/Chests', icon: <Zap size={16} className="text-purple-400" /> },
    { value: 'craft', label: 'Crafting', icon: <Zap size={16} className="text-purple-400" /> },
    { value: 'boss', label: 'Boss Drops', icon: <Zap size={16} className="text-purple-400" /> },
  ];

  // Helper function to determine if an item matches the location filter
  const matchesLocationFilter = (itemDescription: string, locationType: LocationType): boolean => {
    ...
    switch (locationType) {
      case 'vendor':
        return lowerDesc.includes('vendor') || lowerDesc.includes('sold') || lowerDesc.includes('purchased') || lowerDesc.includes('merchant') || lowerDesc.includes('shop');
      case 'quest':
        return lowerDesc.includes('quest') || lowerDesc.includes('reward') || lowerDesc.includes('completing');
      case 'loot':
        return lowerDesc.includes('loot') || lowerDesc.includes('chest') || lowerDesc.includes('found') || lowerDesc.includes('strongbox');
      case 'craft':
        return lowerDesc.includes('craft') || lowerDesc.includes('forge') || lowerDesc.includes('forged');
      case 'boss':
        return lowerDesc.includes('boss') || lowerDesc.includes('defeat') || lowerDesc.includes('dropped') || lowerDesc.includes('looted from');
      default:
        return true;
    }
  };
  ...
  const filteredEquipmentData = useMemo(() => {
    ...
    if (selectedLocation !== 'all') {
      filtered = filtered.filter(item => {
        ...
        return (
          (item.bestInSlot && matchesLocationFilter(item.bestInSlot, selectedLocation)) ||
          (item.secondaryOption && matchesLocationFilter(item.secondaryOption, selectedLocation)) ||
          (item.tertiaryOption && matchesLocationFilter(item.tertiaryOption, selectedLocation))
        );
      });
    }
    ...
    <div className="min-h-screen bg-gray-900 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-6">
          ...
          <p className="text-sm text-gray-500">Optimal build guide for Baldur's Gate 3</p>
        </header>
        
        {/* Ability Scores Section */}
        <div className="mb-6">
          <h2 className="text-xl font-semibold text-purple-300 mb-3">Recommended Ability Scores</h2>
          <AbilityScores scores={selectedClass.abilityScores} isMobile={isMobile} />
        </div>
        ...
        {selectedTab === 'armor' ? (
                  ...
                  {isMobile ? (
                    ...
                    >
                      ...
                      <span className="truncate max-w-[150px]">
                        {selectedSlot === 'all' ? 'All Equipment Slots' : selectedSlot}
                      </span>
                ...
                <div className="flex items-center gap-2">
                  {isMobile ? (
                    ...
                    >
                      ...
                      <span className="truncate max-w-[150px]">
                        {locationOptions.find(loc => loc.value === selectedLocation)?.label || 'All Locations'}
                      </span>
                      <ChevronDown size={16} className="text-purple-400" />
                    </button>
                  ) : (
          ...
          <div className="bg-gray-800 rounded-lg shadow-md overflow-hidden border border-purple-900">
            <div className="p-4 bg-gray-800 border-b border-gray-700">
              <h2 className="text-lg font-medium text-purple-300">Recommended Spells</h2>
              <p className="text-sm text-gray-400 mt-1">Spells marked with <Zap size={14} className="text-purple-400 inline" /> are highly recommended for this build</p>
            </div>
            <div className="p-4">
              <SpellsTable spells={selectedClass.spells} isMobile={isMobile} />
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end justify-center">
          ...
          >
            <div className="sticky top-0 bg-gray-800 p-4 border-b border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-medium text-white">Select Equipment Slot</h3>
              <button 
      ...
      {isMobile && showLocationDrawer && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end justify-center">
          ...
          >
            <div className="sticky top-0 bg-gray-800 p-4 border-b border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-medium text-white">Select Location Type</h3>
              <button 
